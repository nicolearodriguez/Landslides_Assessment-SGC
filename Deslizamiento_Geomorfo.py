...
input= data_path+'/Pre_Proceso/SubunidadesGeomorf.tif'
...
Input1=input
Input2=outfnD
Output= data_path+'/Pre_Proceso/DeslizamientosGeomorfo.tif'
...
alg="native:rasterlayerzonalstats"
input=Output
output= data_path+'/Pre_Proceso/DeslizamientosGeomorfoEstadistica.csv'
params={'INPUT':input,'BAND':1,'ZONES':input,'ZONES_BAND':1,'REF_LAYER':0,'OUTPUT_TABLE':output}
processing.run(alg,params)
...
FILE_NAME = data_path+'/Pre_Proceso/DeslizamientosGeomorfoEstadistica.csv'
DF_DeslizamientosGeomorfoEstadistica = pd.read_csv(FILE_NAME, delimiter=",",encoding='latin-1')
...
for i in range(0,len(atributos)):
    ...
    DF_Susceptibilidad.loc[i,'Mov']=DF_DeslizamientosGeomorfoEstadistica.loc[DF_DeslizamientosGeomorfoEstadistica['zone']==ID]['count'].sum()
...
DF_Susceptibilidad.reset_index().to_csv(data_path+'/02 PreProceso/DF_SusceptibilidadGeomorfo.csv',header=True,index=False)
