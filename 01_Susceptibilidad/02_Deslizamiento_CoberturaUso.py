...
rasterfile = data_path + '/Pre_Proceso/CoberturaUso.tif'
...
if Deslizamientos.wkbType()== QgsWkbTypes.Point:
    ...
    output = data_path+'/Pre_Proceso/ValoresCobertura.shp'
    params = {'INPUT': Deslizamientos, 'RASTERCOPY': rasterfile, 'COLUMN_PREFIX': 'Id_Condici', 'OUTPUT': output}
    processing.run(alg, params)
    
    ValoresRaster = QgsVectorLayer(data_path + '/Pre_Proceso/ValoresCobertura.shp', 'ValoresUGS')
    ...
    Deslizamiento_Condicion = data_path+'/Pre_Proceso/DeslizamientosCobertura.tif'
    ...
    Estadisticas_Deslizamiento = data_path+'/Pre_Proceso/DeslizamientosCoberturaEstadistica.csv'
...
uniquevalues = DF_CoberturayUsosEstadistica["zone"].unique()
...
DF_Susceptibilidad.reset_index().to_csv(data_path + '/Pre_Proceso/DF_SusceptibilidadCoberturayUsos.csv', header = True, index = False)
...
params = {
    'INPUT_RASTER': Ipunt1, 'RASTER_BAND': 1, 'INPUT_TABLE': Input2, 'MIN_FIELD': 'ID', 'MAX_FIELD': 'ID',
    'VALUE_FIELD': 'Wf', 'NO_DATA': -9999, 'RANGE_BOUNDARIES': 2, 'NODATA_FOR_MISSING': False, 'DATA_TYPE': 5, 'OUTPUT': Output}
processing.run(alg, params)

iface.addRasterLayer(data_path + '/Resultados/CoberturayUsosWf.tif', "CoberturayUsosWf")
