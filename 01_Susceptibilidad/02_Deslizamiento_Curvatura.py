...
if Deslizamientos.wkbType()== QgsWkbTypes.Point:
    ...
    Deslizamiento_Condicion = data_path+'/Pre_Proceso/DeslizamientosCurvatura.tif'
    ...
    Estadisticas_Deslizamiento = data_path+'/Pre_Proceso/DeslizamientosCurvaturaEstadistica.csv'
...
params = {
    'INPUT': rasterfile, 'BAND': 1, 'ZONES': rasterfile,
    'ZONES_BAND': 1, 'REF_LAYER': 0, 'OUTPUT_TABLE': output}
processing.run(alg, params)
...
for i in range(1, len(percentiles)):
    ...
    DF_Susceptibilidad.loc[i, 'CLASE'] = DF_CurvaturaEstadistica.loc[
        (DF_CurvaturaEstadistica['zone'] >= Min) & (DF_CurvaturaEstadistica['zone'] < Max)]['count'].sum()
