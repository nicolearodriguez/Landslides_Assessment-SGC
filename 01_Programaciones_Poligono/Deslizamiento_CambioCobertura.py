...
input = data_path+'/Pre_Proceso/CambioCobertura.tif'
output = data_path+'/Pre_Proceso/CoberturaEstadistica.csv'
...
Input1 = data_path+'/Pre_Proceso/CambioCobertura.tif'
Input2 = data_path+'/Pre_Proceso/Deslizamientos.shp'
Output = data_path+'/Pre_Proceso/DeslizamientosCobertura.tif'
...
alg="native:rasterlayerzonalstats"
input = data_path+'/Pre_Proceso/DeslizamientosCobertura.tif'
output = data_path+'/Pre_Proceso/DeslizamientosCoberturaEstadistica.csv'
params={'INPUT':input,'BAND':1,'ZONES':input,'ZONES_BAND':1,'REF_LAYER':0,'OUTPUT_TABLE':output}
processing.run(alg,params)
...
FILE_NAME = data_path+'/Pre_Proceso/DeslizamientosCoberturaEstadistica.csv'
DF_DeslizamientosCoberturaEstadistica = pd.read_csv(FILE_NAME, delimiter=",",encoding='latin-1')
#Valores unicos del factor condicionante
FILE_NAME = data_path+'/Pre_Proceso/CoberturaEstadistica.csv'
...
for i in range(0,len(atributos)):
    ...
    DF_Susceptibilidad.loc[i,'Mov']=DF_DeslizamientosCoberturaEstadistica.loc[DF_DeslizamientosCoberturaEstadistica['zone']==ID]['count'].sum()
...
DF_Susceptibilidad.reset_index().to_csv(data_path+'/Pre_Proceso/DF_SusceptibilidadCobertura.csv',header=True,index=False)

alg="native:reclassifybylayer"
Ipunt1=data_path+'/Pre_Proceso/CambioCobertura.tif'
Input2=data_path+'/Pre_Proceso/DF_SusceptibilidadCobertura.csv'
Output=data_path+'/Resultados/CoberturaWf.tif'
