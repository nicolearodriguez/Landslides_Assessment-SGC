...
DTM = QgsRasterLayer(data_path+'/04_RASTER/DEM_Alos_Mosaic.tif',"DTM")
UGS = data_path+'/05_SHP/SGC/Geologia_Santander.shp'
SubunidadesGeomorf = data_path+'/05_SHP/SGC/UnidadesGeomorfologicas_Santander.shp'
CoberturaUso = data_path+'/05_SHP/GEOHIDRICO/Uso_Cob_2010_2012_IDEAM_SANTANDER.shp'
#CambioCobertura = data_path+'   '
...
for i in range(0,len(atributos)):
    ...
    UGS.selectByExpression(f'"Codigo"=\'{Atri}\'', QgsVectorLayer.SetSelection) #Se hace la selección en la capa de la caracteristica en cuestión
...
for i in range(0,len(atributos)):
    ...
    DF_Raster.loc[i,'Caract']= Atri
    DF_Raster.loc[i,'ID']= i+1
    CoberturaUso.selectByExpression(f'"CODIGO"=\'{Atri}\'', QgsVectorLayer.SetSelection)
...
#    DF_Raster.loc[i,'Caract']= Atri
#    DF_Raster.loc[i,'ID']= i+1
#    CambioCobertura.selectByExpression(f'"   "=\'{Atri}\'', QgsVectorLayer.SetSelection)
#    
#    #Se reemplazan los id del atributo seleccionada
#    selected_fid = []
...
#params= {'INPUT':Shape,'FIELD': Field,'BURN':0,'UNITS':1,'WIDTH':xsize,'HEIGHT':ysize,'EXTENT': "%f,%f,%f,%f"% (xmin, xmax, ymin, ymax),'NODATA':0,'OPTIONS':'','DATA_TYPE':5,'INIT':None,'INVERT':False,'OUTPUT': Raster}
#processing.run(alg,params)
#
